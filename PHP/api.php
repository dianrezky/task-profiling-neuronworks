...
class Implementationmachinelearning
{
    ...
    function nilaiGrafikair($air)
    {
        ...
        if (airNormal($air) != 0) {
            echo "Normal (" . airNormal($air) . ")";
            echo "<br>";
        }
        if (airBanyak($air) != 0) {
            echo "Banyak (" . airBanyak($air) . ")";
            echo "<br>";
        }
        echo "<br>";
    }
    ...
    function nilaiGrafikwaktu($waktu)
    {
        ...
        if (waktuSedang($waktu) != 0) {
            echo "Sedang (" . waktuSedang($waktu) . ")";
            echo "<br>";
        }
        if (waktuLambat($waktu) != 0) {
            echo "Lambat (" . waktuLambat($waktu) . ")";
            echo "<br>";
        }
        echo "<br>";
    }
    ...
    function nilaiGrafikberas($beras)
    {
        ...
        if (berasSedang($beras) != 0) {
            echo "Sedang (" . berasSedang($beras) . ")";
            echo "<br>";
        }
        if (berasBanyak($beras) != 0) {
            echo "Banyak (" . berasBanyak($beras) . ")";
            echo "<br>";
        }
        echo "<br>";
    }
    ...
    public function hasilFuzzifikasi($suhu, $air, $beras, $waktu)
    {

        nilaiGrafiksuhu($suhu);
        nilaiGrafikair($air);
        nilaiGrafikberas($beras);
        nilaiGrafikwaktu($waktu);
    }
}
...
class Financial
{
    ...
    public function addRecord($date, $description, $amount)
    {
        $record = [
            'date' => $date,
    ...
    public function deleteRecord($index)
    {
        ...
        // Lakukan pengurutan ulang jika diperlukan
        $this->records = array_values($this->records);
    }

    public function filterRecords($filter)
    {
        $filteredRecords = [];
        foreach ($this->records as $record) {
            if (
                $record['amount'] >= $filter['minAmount'] && $record['amount'] <= $filter['maxAmount'] &&
                $record['date'] >= $filter['startDate'] && $record['date'] <= $filter['endDate'] &&
                stripos($record['description'], $filter['keyword']) !== false
            ) {
                $filteredRecords[] = $record;
            }
        }
    ...
    public function saveRecordsToFile($filename)
    {
